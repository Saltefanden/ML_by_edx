...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    n = X.shape[0]
    probabilities_Y = np.choose(Y,probabilities)
    softmax_loss = -1/n * np.sum(np.log( probabilities_Y )) # -log = 0 if the certainty is 100%, else it goes to infinity as certainty nears 0%
    regularization = lambda_factor/2 * np.sum( np.square(theta) )
    return softmax_loss + regularization
